const hiddenClass = 'selling-plan-widget--hidden';
...
class SellingPlansWidget {
  ...
  */
  handleSellingPlanFormVisibility() {
    console.log('made it to handleSellingPlanFormVisibility');
    ...
    const sellingPlanFormsForUnselectedVariants =
      this.shopifySection.querySelectorAll(
        `.selling-plan-widget:not([data-variant-id="${this.variantIdInput.value}"])`,
      );
  ...
  */
  listenToVariantChange() {
    console.log('made it to listenToVariantChange');
  ...
  listenToAddToCartForms() {
    console.log('made it to listenToAddToCartForms');
    this.addToCartForms.forEach((addToCartForm) => {
